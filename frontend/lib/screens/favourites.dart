...
/// Favourites home — same poster-rail feel as Home, organised by kind
/// (Movies / Series / Anime) instead of Popular / Last added.
      ...
      body: TvBackScope(
        ...
        body: feed.when(
          ...
          error: (e, _) => EmptyState(
            ...
            onRefresh: () async => ref.invalidate(favoritesFeedProvider),
          ),
          data: (data) {
            if (data.isEmpty) {
              return const EmptyState(
                message:
                    'No favourites yet. Open a title and tap the heart to save it here.',
                showSettings: false,
              );
            }
            ...
            return ClippedOverlay(
              child: ListView(
                ...
                children: [
                  FeaturedBanner(
                    key: const ValueKey('favourites-banner'),
                    items: bannerItems,
                  ),
                  _FavRow(title: 'Movies', items: data.movies),
                  _FavRow(title: 'Series', items: data.series),
                  _FavRow(title: 'Anime', items: data.anime),
                ],
              ),
            );
          },
...
class _FavRow extends StatelessWidget {
  const _FavRow({required this.title, required this.items});
